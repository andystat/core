...
if (isActionAccessible($guid, $connection2, "/modules/Attendance/attendance_take_byCourseClass.php")==FALSE) {
    ...
    //$sql = "SELECT gibbonCourseClass.gibbonCourseClassID AS value, gibbonCourse.nameShort AS name, CONCAT(gibbonCourseClass.nameShort, '.', gibbonCourseClass.nameShort) AS class FROM gibbonCourseClassPerson JOIN gibbonCourseClass ON (gibbonCourseClassPerson.gibbonCourseClassID=gibbonCourseClass.gibbonCourseClassID) JOIN gibbonCourse ON (gibbonCourseClass.gibbonCourseID=gibbonCourse.gibbonCourseID) WHERE gibbonCourse.gibbonSchoolYearID=:gibbonSchoolYearID AND gibbonPersonID=:gibbonPersonID AND gibbonCourseClass.attendance='Y' ORDER BY name" ;
    ...
    $row = $form->addRow();
        $row->addHeading("CHOOSE CLASS");
        
    $row = $form->addRow();
        $row->addLabel('gibbonClassID', __('Choose Class'));
        $row->addSelectClass("gibbonClassID", $_SESSION[$guid]['gibbonSchoolYearID'])->selected($gibbonCourseClassID);
            
    $row = $form->addRow();
        $row->addLabel('currentDate', __('Date'));
        $row->addDate("currentDate")->setValue(dateConvertBack($guid, $currentDate));
